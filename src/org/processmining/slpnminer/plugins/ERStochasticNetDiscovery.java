...
@Plugin(name = "Discover Stochastic net with Entropic Relevance",
        returnLabels = { "stochastic Petri net" },
        returnTypes = { StochasticNetImpl.class},
        parameterLabels = { "net", "log"})
public class ERStochasticNetDiscovery {
    ...
    private static AcceptingPetriNet apnet;

    @UITopiaVariant(affiliation = UITopiaVariant.EHV, author = "Tian Li", email = "t.li@bpm.rwth-aachen.de", pack = "WeightEstimation")
    @PluginVariant(requiredParameterLabels = {0, 1})
    public StochasticNetImpl calculateTS(PluginContext context, XLog log, Petrinet net) throws ConnectionCannotBeObtained, ParseException {
		
    	
    	Marking initialMarking = guessInitialMarking(net);
    	apnet = new AcceptingPetriNetImpl(net, initialMarking);
		logSize = log.size();
    ...
    public StochasticNetImpl calculateTS(PluginContext context, XLog log, Petrinet net, Marking state, PetrinetSemantics semantics)
    ...
    private StochasticNetImpl buildAndConnect(PluginContext context,
                                     XLog log,
                                     PetrinetGraph net,
                                     Marking initial,
...
//            	get trace variant
            ...
            //construct equations system
	            ...
	            String assis1 = "Solve";
	            String eqSys = eq.getEqStr(another_ts, tm, tProbMap, combiToPetri, reachableState); 
	            try {
//	            	String isolatedVar = IsolateVariable.getIsolatedVar(assis1+eqSys);	            	            	
	            	ExecutorService executorService = Executors.newSingleThreadExecutor();
	        		String isolatedVar = null;
	        		Future<String> future1 = executorService.submit(() ->  IsolateVariable.getIsolatedVar(assis1+eqSys));
	        		
	        		try {
	        		    //设置超时时间
	        			isolatedVar = future1.get(30, java.util.concurrent.TimeUnit.SECONDS);
        ...
        mapFlag = true;
        MultivariateFunction fEntropicRelevance = new MultivariateFunction() {
            public double value(double[] x) {
                return getEntropicRelevance(x);
            }
        };
		...
		double[] sigma = new double[modelTransitionNum];
		  Arrays.fill(lowerBound, 0.0001);
			...
			double[] result1 = optimizer.optimize(
					new MaxEval(10000),
			   new ObjectiveFunction(fEntropicRelevance),
			   GoalType.MINIMIZE,
...
//			System.out.print(result1[5]);
			...
			StochasticNetImpl result = new StochasticNetImpl(apnet.getNet().getLabel());
			...
			for (Place inputPlace : apnet.getNet().getPlaces()) {
				Place resultPlace = result.addPlace(inputPlace.getLabel());
				input2result.put(inputPlace, resultPlace);
			}
			for (Transition inputTransition : apnet.getNet().getTransitions()) {
//				get transition weight
				String tr = tmap.get(inputTransition.getLocalID().toString());
			...
			for (PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode> edge : apnet.getNet().getEdges()) {
...
//		        System.out.println("get id of trans: "+t.getLocalID()+"  "+t.getLabel());
    ...
    public static double getEntropicRelevance(double[] t){
